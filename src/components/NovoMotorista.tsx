...
export function NovoMotorista({ openNovoMotorista, openChangeNovoMotorista, onRefresh }: { openNovoMotorista: boolean, openChangeNovoMotorista: (v: boolean) => void, onRefresh: () => void }) {

    const [motorista, setMotorista] = useState<Motorista[]> ([]);
    const [motoristaNovo, setMotoristaNovo] = useState({
      nome: "",
      contato: "",
      secretariaId: "b4279091-986c-4598-93ed-eb2385606394"
    });

    const [showDialogNovoMotorista, setShowDialogNovoMotorista] = useState(false);

    function openDialogNovoMotorista(motorista: Motorista) {
    ...
    async function cadastroMotorista(e: React.FormEvent) {
      ...
      try {
          ...
          setMotoristaNovo(motoristaNovo);
          onRefresh();
      } catch (error) {
    ...
    return (
            <AlertDialog open={openNovoMotorista} onOpenChange={openChangeNovoMotorista}>
              <AlertDialogContent className="w-full">
                <AlertDialogHeader>
                  <AlertDialogTitle>Gostaria de cadastrar um novo motorista?</AlertDialogTitle>
                ...
                <form onSubmit={cadastroMotorista}>
                  <div className='flex grid-cols-2 gap-4'>
                      <div className='flex flex-col gap-2 w-1/3 justify-items-start'>
                          ...
                          onChange={(e) => setMotoristaNovo(prev => ({ ...prev, nome: e.target.value}))}
                          placeholder='Nome'
                          ...
                          onChange={(e) => setMotoristaNovo(prev => ({ ...prev, contato: e.target.value}))}
                          placeholder='Contato'
                          className='w-full text-gray-500 rounded mb-2 border border-gray-300'/></p>
                          <p><span>Secretaria: </span>
                          <Input 
                          type="text"
                          value={motoristaNovo?.secretariaId}
                          onChange={(e) => setMotoristaNovo(prev => ({ ...prev, secretaria: e.target.value}))}
